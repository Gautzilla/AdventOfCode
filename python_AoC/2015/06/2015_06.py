...
class Instruction:
	def __init__(self, action: Action, top_left: tuple[int,int], bottom_right: tuple[int,int]):
		self.action = action
	...
	def from_str(cls, s: str):
		match = re.search(r"(?P<action>\D+)(?P<top_left>\d+,\d+) through (?P<bottom_right>\d+,\d+)", s)
...
class Square:
	...
	def activate(self, instruction: Instruction) -> None:
		...
		light_action = self.light_actions[action] if self.part == 1 else self.light_actions_part_2[action]
