...
def highest_joltage(batteries: list[int], nb_lit_batteries: int) -> int:
    ...
    for battery in range(nb_lit_batteries):
        ...
        lit_batteries.append(
            max(list(enumerate(batteries))[start_idx:stop_idx], key=lambda b: b[1])
        )
...
def part1(puzzle_input: str) -> int:
    return sum(
        highest_joltage([int(b) for b in line], 2) for line in puzzle_input.splitlines()
    )


def part2(puzzle_input: str) -> int:
    return sum(
        highest_joltage([int(b) for b in line], 12)
