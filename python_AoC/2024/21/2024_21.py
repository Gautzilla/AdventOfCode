...
def path_length(a,b,depth):
	...
	return sum(path_length(*p, depth-1) for p in zip("A" + next_sequence, next_sequence + "A"))
...
def drive_robots(puzzle_input: str, nb_robots: int) -> int:
	...
	for input in puzzle_input.splitlines():
		...
		for c in input:
			...
			cc = c
		output[input] = sum(path_length(*p, nb_robots-1) for p in zip("A" + sequence, sequence))
