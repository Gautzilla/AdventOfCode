...
def search(obstacles, run, runs, visited):
	...
	for d in range(direction-1,direction+2):
		...
		if position2 in obstacles:
			continue
		if (position2, d) in visited or (position2,(d+2)%len(DIRECTIONS)) in visited:
			continue
...
def parse_grid(puzzle_input: str) -> tuple[set[tuple[int,int]], tuple[int,int], tuple[int,int]]:
	start = (0,0)
	end = (0,0)
	obstacles = set()
