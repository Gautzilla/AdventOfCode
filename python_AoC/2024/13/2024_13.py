...
def part2(machines) -> any:
	...
	machines = [(xa, ya, xb, yb, xp+prize_offset, yp+prize_offset) for xa, ya, xb, yb, xp, yp in machines]
	return sum(solve_machine(machine) for machine in machines)
...
def solve(puzzle_input: str) -> tuple[any, any]:
	...
	machines = [parse_machine(input_lines[n:n+3]) for n in range(0, len(input_lines), 3)]
